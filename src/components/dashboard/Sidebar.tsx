...
interface SidebarProps {
  ativo: SidebarItemKey;
  onLogout: () => void;
}

const navItems: Array<{ key: SidebarItemKey; label: string; href: string; icon: string }> = [
  { key: 'dashboard', label: 'Dashboard', href: '/dashboard', icon: '◔' },
  { key: 'members', label: 'Membros', href: '/members', icon: '◌' },
  { key: 'attendance', label: 'Presenças', href: '/attendance', icon: '◍' },
  { key: 'leads', label: 'Leads', href: '/leads', icon: '◎' },
  { key: 'payments', label: 'Pagamentos', href: '/payments', icon: '▣' },
  { key: 'settings', label: 'Definições', href: '/settings', icon: '☰' },
];

export default function Sidebar({ ativo, onLogout }: SidebarProps) {
  return (
    <aside className="fixed left-0 top-0 z-20 flex h-screen w-[260px] flex-col border-r border-[#222] bg-[#0d0d0d]">
      ...
      <nav className="mt-4 flex-1 px-3">
        {navItems.map((item) => {
          const activeItem = item.key === ativo;
          return (
            <Link
              key={item.key}
              href={item.href}
