...
export function AdultsFiltersBar({ value, onChange }: AdultsFiltersBarProps) {
  ...
  return (
    <div className="mb-3">
      <div className="-mx-1 flex items-center gap-1.5 overflow-x-auto px-1 pb-1 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0">
        {[
          { key: 'all', label: 'Todos' },
          ...
          <button key={chip.key} onClick={() => set({ status: chip.key as AdultsFilters['status'] })} style={chipStyle(value.status === chip.key)} className="shrink-0 whitespace-nowrap">
            {chip.label}
          </button>
        ))}
        <button
          onClick={() => setShowMore((v) => !v)}
          style={chipStyle(showMore)}
    ...
    <div className="mb-3">
      <div className="-mx-1 flex items-center gap-1.5 overflow-x-auto px-1 pb-1 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0">
        {[
          ...
          <button key={chip.key} onClick={() => set({ behavior: chip.key as KidsFilters['behavior'] })} style={chipStyle(value.behavior === chip.key)} className="shrink-0 whitespace-nowrap">
            {chip.label}
          </button>
        ))}
        <button
